...
BezierGLWidget::BezierGLWidget(QWidget* parent)
...
BezierGLWidget::~BezierGLWidget() {
    qDebug() << "BezierGLWidget destructor called";
}

void BezierGLWidget::initializeGL() {
    ...
    connect(updateTimer, &QTimer::timeout, this, QOverload<>::of(&BezierGLWidget::update));
    updateTimer->start(16); // ~60 FPS
}

void BezierGLWidget::resizeGL(int w, int h) {
    glViewport(0, 0, w, h);
...
QVector3D BezierGLWidget::deCasteljau(float t, const QVector<QVector3D>& points) {
    if (points.size() == 1)
        return points[0];
...
void BezierGLWidget::drawBezier() {
...
void BezierGLWidget::drawRevolvedSurface() {
    if (controlPoints.size() < 2)
        return;
...
void BezierGLWidget::mousePressEvent(QMouseEvent* event) {
...
void BezierGLWidget::mouseMoveEvent(QMouseEvent* event) {
...
void BezierGLWidget::mouseReleaseEvent(QMouseEvent* event) {
    Q_UNUSED(event);
    selectedPointIndex = -1; // Deselect the control point
}

void BezierGLWidget::keyPressEvent(QKeyEvent* event) {
